...
BEGIN
	...
	BEGIN TRY
		...
		CASE UPPER(TRIM(prd_line))
			...
			WHEN 'T' THEN 'Touring'
			ELSE 'n/a' 
		END prd_line, -- Map product line to descriptive values
		prd_start_dt,
		LEAD(prd_start_dt) OVER(PARTITION BY prd_key ORDER BY prd_start_dt) - 1 prd_end_dt  -- Calculate end date as one day before the next start date
		...
		ELSE 'n/a' -- Normalize gender values and hanle unknown cases
		END gen
		...
		(
		SELECT
		REPLACE(cid, '-', '') cid, -- Removed invalid valid
		CASE
